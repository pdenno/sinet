...
** Add MJP 
    This entails watching the execution times, maybe doing GPtips like regression.
** Add Causal
...
1) Use genetic operators to produce more varied individuals, including some that 
   might parse the log well enough to get an exceptional msg table with markings that
   can guide design.

2) Instead of next-ordinary, use next-ordinary-on-job. That will provide a parse
...
The winning individual came real close after two iterations. 
